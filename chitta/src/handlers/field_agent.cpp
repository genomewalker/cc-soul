// Agent registry RPC handlers — bodies for declarations in
// chitta/include/chitta/rpc/handlers/field_agent.hpp.
...
ToolResult FieldRpcHandler::tool_agent_upsert(const json& params) {
...
ToolResult FieldRpcHandler::tool_agent_get(const json& params) {
    if (!field_store_) return ToolResult::error("chitta-field store unavailable");
...
ToolResult FieldRpcHandler::tool_agent_list() {
    if (!field_store_) return ToolResult::error("chitta-field store unavailable");
...
ToolResult FieldRpcHandler::tool_agent_disable(const json& params) {
    if (!field_store_) return ToolResult::error("chitta-field store unavailable");
