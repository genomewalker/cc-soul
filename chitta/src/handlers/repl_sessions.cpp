// repl_sessions RPC handlers — bodies for declarations in
// chitta/include/chitta/rpc/handlers/repl_sessions.hpp.

#include "../../include/chitta/rpc/field_handler.hpp"

namespace chitta {

ToolResult FieldRpcHandler::tool_repl_session_get(const json& params) {
...
ToolResult FieldRpcHandler::tool_repl_session_set(const json& params) {
    if (!field_store_) return ToolResult::error("chitta-field store unavailable");
    std::string session_id     = params.value("session_id", "");
...
ToolResult FieldRpcHandler::tool_repl_session_delete(const json& params) {
    if (!field_store_) return ToolResult::error("chitta-field store unavailable");
    std::string session_id = params.value("session_id", "");
    if (session_id.empty()) return ToolResult::error("session_id required");
...
ToolResult FieldRpcHandler::tool_repl_execute(const json& params) {
    if (!field_store_) return ToolResult::error("chitta-field store unavailable");
    std::string session_id = params.value("session_id", "");
...
ToolResult FieldRpcHandler::tool_repl_session_list() {
    if (!field_store_) return ToolResult::error("chitta-field store unavailable");
