// drift_recon RPC handlers — bodies for declarations in
// chitta/include/chitta/rpc/handlers/drift_recon.hpp.

#include "../../include/chitta/rpc/field_handler.hpp"

namespace chitta {

ToolResult FieldRpcHandler::tool_set_evidence_type(const json& params) {
    ...
    if (evidence_type.empty()) return ToolResult::error("evidence_type is required");
    ...
    return ToolResult::ok("Evidence type set to '" + evidence_type + "'",
        {{"id", id_str}, {"evidence_type", evidence_type}});
}

ToolResult FieldRpcHandler::tool_get_evidence_type(const json& params) {
    uint64_t id = extract_id(params);
    if (id == 0) return ToolResult::error("id is required");
    ...
    return ToolResult::ok(evidence_type,
        {{"id", id_str}, {"evidence_type", evidence_type}});
}

ToolResult FieldRpcHandler::tool_labile_memories(const json& params) {
...
ToolResult FieldRpcHandler::tool_reconsolidate(const json& params) {
    uint64_t id = extract_id(params);
    if (id == 0) return ToolResult::error("id is required");
    ...
    return ToolResult::ok("Reconsolidated memory #" + id_str,
