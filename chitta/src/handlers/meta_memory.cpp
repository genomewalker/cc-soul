// meta_memory RPC handlers — bodies for declarations in
// chitta/include/chitta/rpc/handlers/meta_memory.hpp.

#include "../../include/chitta/rpc/field_handler.hpp"

namespace chitta {

ToolResult FieldRpcHandler::tool_record_surprise(const json& params) {
    json p;
...
ToolResult FieldRpcHandler::tool_query_surprises(const json& params) {
    json p;
...
ToolResult FieldRpcHandler::tool_get_blind_spots(const json& params) {
    json p;
    if (params.contains("realm")) p["realm"] = params["realm"];
...
ToolResult FieldRpcHandler::tool_surprise_stats(const json&) {
...
ToolResult FieldRpcHandler::tool_register_debt(const json& params) {
    json p;
...
ToolResult FieldRpcHandler::tool_resolve_debt(const json& params) {
...
ToolResult FieldRpcHandler::tool_defer_debt(const json& params) {
    uint64_t debt_id = params.value("debt_id", uint64_t(0));
    if (debt_id == 0) return ToolResult::error("debt_id is required");
...
ToolResult FieldRpcHandler::tool_query_debts(const json& params) {
    json p;
...
ToolResult FieldRpcHandler::tool_get_fragile_decisions(const json& params) {
    json p;
...
ToolResult FieldRpcHandler::tool_debt_stats(const json&) {
...
ToolResult FieldRpcHandler::tool_record_feedback(const json& params) {
    json p;
...
ToolResult FieldRpcHandler::tool_get_source_weights(const json& params) {
    json p;
    if (params.contains("domain")) p["domain"] = params["domain"];
...
ToolResult FieldRpcHandler::tool_update_source_weight(const json& params) {
    json p;
    p["source"] = params.value("source", "");
    p["domain"] = params.value("domain", "general");
...
ToolResult FieldRpcHandler::tool_integration_stats(const json&) {
