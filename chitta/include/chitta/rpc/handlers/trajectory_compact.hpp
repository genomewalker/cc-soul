...
// Trajectory compaction: attention-weighted turn selection for agent context transfer.
//
// Instead of LLM summarization (lossy, expensive), selects the most task-relevant
// turns from a transcript using embedding similarity + MAD adaptive thresholding.
//
// References:
//   [1] Ramp Labs (2025). "Latent Briefing: KV Cache Compaction for Multi-Agent
//       LLM Orchestration." — Task-guided query vectors for selecting relevant
//       tokens from orchestrator trajectory; global scoring across shared tokens.
//   [2] Leys, C. et al. (2013). "Detecting outliers: Do not use standard deviation
//       around the mean." J. Experimental Social Psychology. — MAD as robust
//       threshold for non-Gaussian score distributions.
//   [3] Koylan (2025). "Building a Personal Brain OS with Claude." — Progressive
//       disclosure pattern: L1 router → L2 module → L3 data.
//
// Algorithm:
//   1. Parse transcript → turns (reuses field_session.hpp parser logic)
//   2. Embed task description as query vector  [1: task-guided query]
//   3. Score each turn: cosine_sim(turn_embedding, task_embedding)
//   4. Apply MAD threshold: keep turns > median + k * MAD  [2: robust outlier detection]
//   5. Enforce token budget: drop lowest-scoring surplus, backfill if under
//   6. Return chronologically ordered selected turns (lossless subset)
...
#include <fstream>
    ...
    ToolResult tool_trajectory_compact(const json& params) {
