...
// Context compaction: memory-aware message pruning for conversation windows.
//
// Phases:
//   1. Score each turn: recency + semantic_sim(query) + (1 - memory_coverage)
//   2. Greedy keep by token budget; system messages always kept
//   3. [distill_novel] For novel drops (low coverage), distill into soul memories
//   4. Return kept messages verbatim in original order + stats
