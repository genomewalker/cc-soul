...
# CC_SOUL_PLUGIN_DIR, and it falls back to scanning for a *real* cc-soul
# checkout otherwise — so simulate "missing" with an empty chitta-mcp dir
# rather than a nonexistent plugin root, or the fallback chain would find
# this repo's own (present) registry and defeat the test.
