...
def chitta_recall(query: str, limit: int) -> list[dict]:
    ...
    try:
        ...
        # recall printed an error rather than JSON; this probe records no hits.
        return []
...
def dam_update(
    X: np.ndarray, s: np.ndarray, beta: float, steps: int
...
def main():
    ...
    for item in TEST_QUERIES:
        ...
        target_label = (
            f"rank {r['target_rank']}"
            if r["target_rank"] is not None
            else f"{RED}not in pool{RESET}"
        )
        ...
        for beta, pb in r["per_beta"].items():
            ...
            print(
                f"  β={beta:>3}  {flip_tag}  steps={pb['n_steps']:>2}  δ={pb['final_delta']:.5f}"
    ...
    print(
        f"  target hits       : {target_hits}/{len(TEST_QUERIES) * len(BETAS)}  (DAM converged to correct memory)"
    )
