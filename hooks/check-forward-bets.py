...
def recall_bets(limit=20):
    ...
    r = subprocess.run(
        ["chitta", "recall", "--query", "status:open forward bet prediction",
         "--realm", GRADE_REALM, "--limit", str(limit), "--json"],
...
def main():
    ...
    if a.resolve:
        ...
        subprocess.run(["chitta", "remember", "--content", content,
                        "--realm", GRADE_REALM, "--type", "wisdom",
                        "--tags", "bet-resolved", "--visibility", "1"],
