...
def _hashish(t): return any(c in 'abcdef' for c in t) and any(c.isdigit() for c in t)
def _valid_abs(p):
    if _urlish.search(p): return False
    b = p[1:]
...
def atoms(c):
    ...
    for m in _kv.finditer(c):
        p = m.group(1)
        if len(p) >= 8 and '/' in p and not _urlish.search(p): out.add(p)
    for p in _abs.findall(c):
        if len(p) >= 12 and _valid_abs(p): out.add(p)
...
def load_mems(path):
    ...
    def content(v): return (v.get("content") or "") if isinstance(v, dict) else str(v or "")
...
def main():
    ...
    for mid, c in mems.items():
        for a in atoms(c): idx[a].add(mid)
    ...
    for tf in glob.glob(tglob):
        for q, a in turns(tf):
            ...
            for atom in reply_only:
                mids = idx.get(atom, ())
                if not mids or len(mids) > max_df: continue   # unknown or too common
                for mid in mids:
                    hit[mid].add(atom)
            for mid, ats in hit.items():
                labels.append({"query": q[:500], "relevant_id": mid,
    ...
    if labels:
        ...
        print(f"labels/query: median={statistics.median(per_q.values()):.0f} "
