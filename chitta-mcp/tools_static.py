...
Generator: chitta-mcp/sync_tools.py (console script `chitta-mcp-sync`), which
reads the running daemon's tools/list response and rewrites this whole file.
Hand edits are lost on the next sync, and the file is excluded from ruff
(see ruff.toml) for the same reason.
...
COMPOSITE_TOOLS = [
    Tool(
        name="advanced",
        description="Gateway to hidden/advanced tools. Use action='list' to see every hidden tool (about 250), or call directly with tool='<name>' and arguments={...}. Example: {\"tool\": \"pin_memory\", \"arguments\": {\"id\": 123}}",
        inputSchema={
            "type": "object",
            "properties": {
                "action": {
