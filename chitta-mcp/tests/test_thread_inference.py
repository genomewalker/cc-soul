...
from thread_inference import extract_user_turns, fingerprint, infer, snapshot_user_turns  # noqa: E402
...
class TranscriptFormatTests(unittest.TestCase):
    ...
    def test_extracts_claude_user_turns(self):
        path = self._write([
            {"type": "user", "message": {"content": [{"type": "text", "text": "work on cc-soul bridge"}]}},
            {"type": "assistant", "message": {"content": [{"type": "text", "text": "ok"}]}},
            {"type": "user", "message": {"content": "track Claude sessions too"}},
        ])
        self.assertEqual(extract_user_turns(path), [
            "work on cc-soul bridge", "track Claude sessions too",
        ])
    ...
    def test_loads_bounded_stop_snapshot(self):
        ...
        Path(path).write_text(json.dumps({
            "user_turns": ["fix stop hook", "run tests"]
        }))
    ...
    def test_bounded_stop_snapshot_dedups_without_recleaning(self):
        path = self._write([])
        Path(path).write_text(json.dumps({
            "user_turns": ["run tests", "run tests", "fix stop hook"]
        }))
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                thread_id = task_ledger.thread_create(
                    "shared session leases", "project:test",
                ...
                result = infer(
                    path, "project:test", session_id="codex-other",
                    client="codex", project_dir="/tmp/project",
                )
                ...
                self.assertEqual(task_ledger.session_get("codex-other")["thread_id"],
                                 result["thread_id"])
            finally:
