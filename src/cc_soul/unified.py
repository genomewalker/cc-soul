...
class SoulContext:
    ...
    ambient_domains: Set[str] = field(default_factory=set)   # Background domains (don't surface, still influence)
...
def _format_woven(ctx: SoulContext) -> str:
    ...
    if len(ctx.domains) > 1 or ambient_richness >= 3 or (ctx.resonance_patterns and ctx.resonance_patterns[0][1] > 0.6):
