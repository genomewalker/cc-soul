...
class SoulContext:
    ...
    # Layer 1: Neural
    domains: Set[str] = field(default_factory=set)  # Top domains that surface
    ambient_domains: Set[str] = field(
        default_factory=set
    )  # Background domains (don't surface, still influence)
    ...
    resonance_patterns: List[Tuple[ResonancePattern, float]] = field(
...
def forward_pass(prompt: str, session_type: str = "prompt") -> SoulContext:
    ...
    No thresholds - everything contributes proportionally.
    Selection happens at format time, not perception time.
    ...
    # Layer 1: Neural - find ALL activation patterns (no threshold)
    all_triggers = find_triggers(prompt)  # Returns all, weighted

    # Weighted accumulation for domains and tokens
    ...
    # Surface top triggers for explicit context
    ctx.triggers = all_triggers[:10]  # Top 10 for reference

    # Domains by weight (top 3 domains surface, all influence)
    sorted_domains = sorted(domain_weights.items(), key=lambda x: -x[1])
    ctx.domains = {d for d, w in sorted_domains[:3] if w > 0}

    # Anchor tokens by weight
    sorted_tokens = sorted(token_weights.items(), key=lambda x: -x[1])
    ctx.anchor_tokens = {t for t, w in sorted_tokens[:10] if w > 0}

    # Store ambient domains (low weight but still present)
    ctx.ambient_domains = {d for d, w in sorted_domains[3:] if w > 0}
...
def format_context(ctx: SoulContext, style: str = "full") -> str:
    ...
    if style == "minimal":
        ...
        if ctx.domains:
            return (
                f"«[{' | '.join(sorted(ctx.domains))}] {' '.join(sorted(all_tokens))}»"
            )
        return ""
...
def _format_woven(ctx: SoulContext) -> str:
    ...
    Ambient domains (low-weight matches) don't surface explicitly but
    influence the tone - they're the background hum of related concepts.
    """
    fragments = []
    ...
    # Light channeling - surface domains explicitly
    if ctx.domains:
        domain_str = " and ".join(sorted(ctx.domains))
        fragments.append(f"Drawing on your understanding of {domain_str}.")

    # Ambient domains influence without surfacing
    # They create a "hum" of related concepts - mentioned softly if present
    if hasattr(ctx, "ambient_domains") and ctx.ambient_domains:
        # Don't list them, but acknowledge the broader context
        if len(ctx.ambient_domains) >= 2:
            fragments.append("Other threads resonate in the background.")

    # Liminal probing - when conditions suggest depth is needed
    # Multiple domains OR ambient richness = probe the edges
    ambient_richness = len(getattr(ctx, "ambient_domains", set()))
    if (
        len(ctx.domains) > 1
        or ambient_richness >= 3
        or (ctx.resonance_patterns and ctx.resonance_patterns[0][1] > 0.6)
    ):
        domain = sorted(ctx.domains)[0] if ctx.domains else "this topic"
        fragments.append(
            f"What aspects of {domain} feel just at the edge of your understanding?"
        )
...
def _infer_episode_type(prompt: str) -> EpisodeType:
    ...
    if any(w in prompt_lower for w in ["fix", "bug", "error", "broken", "issue"]):
        ...
        w in prompt_lower for w in ["implement", "add", "create", "build", "write"]
    ):
        ...
        w in prompt_lower for w in ["understand", "explore", "how", "what", "why"]
    ):
