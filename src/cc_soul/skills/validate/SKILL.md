...
```python
# Pseudo-process
beliefs = recall_beliefs(domain="architecture")
wisdom = recall_wisdom(query="similar decisions")
intentions = get_active_intentions()

conflicts = []
for belief in beliefs:
    if not decision.aligns_with(belief):
...
if conflicts:
    ...
    record_decision(decision, conflicts, justification)
```
