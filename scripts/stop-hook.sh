...
(
    ...
    LAST_MSG=$(tac "$TRANSCRIPT" | grep -m1 '"role":"assistant"' | jq -r '.content // empty' 2>/dev/null | head -c 2000)
    ...
    if echo "$LAST_MSG" | grep -qiE '\[LEARN\]|\[REMEMBER\]|worth remembering|key insight'; then
        ...
        if [[ -n "$INSIGHT" && ${#INSIGHT} -gt 10 ]]; then
            call_mcp "observe" "$(jq -n --arg c "$INSIGHT" '{category:"discovery",title:($c|.[0:60]),content:$c}')" >/dev/null 2>&1
        fi
    fi
) &
